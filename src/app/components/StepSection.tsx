...
const StepSection = () => {
  ...
  const nextStep = recipe.steps.length + 1;
  ...
  useEffect(() => {
    if (recipe.editableStepIdx > -1 && inputRef.current) {
      inputRef.current.value = recipe.steps[recipe.editableStepIdx];
      inputRef.current.focus();
    }
  }, [recipe.editableStepIdx, recipe.steps]);
    ...
    <>
      {recipe.steps.length < 1 ? (
        ...
        <ol className="text-sm text-foreground list-decimal ml-6 space-y-1">
          {recipe.steps.map((step, idx) => {
            const isEditable = recipe.editableStepIdx === idx;
            ...
            return (
              <li key={step}>
                <div className="grid grid-cols-12 gap-2 items-center">
                  ...
                  <div>
                    <Button
                      onClick={() => dispatch(setEditableStep(idx))}
                      variant="outline"
                      size="icon"
                  ...
                  <div>
                    <Button
                      onClick={() => dispatch(removeStep(idx))}
                      variant="outline"
                      size="icon"
                      className="rounded-3xl text-indigo-600"
                    >
      ...
      <div className="space-y-2">
        <Input
          ...
          placeholder={`Masukan langkah ke ${nextStep}`}
          ref={inputRef}
        />
